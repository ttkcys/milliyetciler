...
export default function AdminKadroEditPage() {
  const router = useRouter();
  const sp = useSearchParams();
  const kadroId = sp.get("id");
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        const res = await fetch(`${API_BASE}/kadros/${kadroId}`, {
          credentials: "include",
          signal: controller.signal,
        });

        if (!res.ok) {
          setError("Kadro bulunamadı");
          setLoading(false);
          return;
        }
        ...
        if (data.image) {
          ...
          setImagePreview(`${STORAGE_BASE}/${data.image}`);
        } else {
          setImagePath("");
          setImagePreview(null);
        }
      } catch (e) {
        if (e.name !== "AbortError") setError("Kadro yüklenirken hata oluştu");
      } finally {
        setLoading(false);
      }
    })();
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      if (!res.ok) {
        const errData = await res.json().catch(() => ({}));
        setError(errData.message || "Kadro güncellenirken hata oluştu.");
        return;
      }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-white/60">Kadro yükleniyor...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen text-white">
      <div className="relative overflow-hidden border-b border-[#333]">
        ...
        <div className="relative mx-auto max-w-7xl px-6 py-12">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-4xl font-bold">Kadro Düzenle</h1>
              <p className="text-white/60 text-lg mt-1">{formData.name || "Bilgileri güncelleyin"}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="mx-auto max-w-4xl px-6 py-10">
        ...
        {success && <div className="mb-6 rounded-lg border border-green-500/30 bg-green-500/10 px-4 py-3 text-sm text-green-300">Güncellendi!</div>}
            ...
            <div className="flex items-start gap-6">
              ...
              <div className="flex-1">
                <label htmlFor="image-upload" className="inline-flex cursor-pointer items-center gap-2 rounded-lg border border-[#333] bg-transparent px-4 py-2 text-sm transition-all duration-300 hover:border-[#ffc451] hover:bg-[#ffc451]/10">
                  ...
                  {uploading ? "Yükleniyor..." : imagePreview ? "Fotoğrafı Değiştir" : "Fotoğraf Seç"}
                </label>
