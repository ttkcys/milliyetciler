...
type Author = {
  ...
  olum: string | null;
  image: string | null;
  // API'de var ama eski tipte yoktu; ekledik (opsiyonel)
  parent?: string | null;
  childs?: any;
};
type AuthorWork = {
  ...
  yil: number | null;
  sayfa: string | null;
  // API zaten döndürüyor; sıralamada kullanacağız
  created_at?: string;
  updated_at?: string;
};
// Kartta hangi mahlas/ID ile geldiğini göstermek için genişlettik
  ...
  try {
    ...
    if (typeof value === "string" && value.trim()) {
      try {
        ...
        if (Array.isArray(parsed)) parsed.forEach(pushVal);
        else pushVal(parsed);
...
export default function YazarDetayPage() {
  ...
  const showLoginRedirect = (authorId: number) => {
    ...
    setToast({
      ...
      message:
        "Yazarları listenize eklemek için giriş yapmalısınız. Birazdan yönlendirileceksiniz…",
      href,
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        // ID -> isim eşleşmesi (tek tek çekiyoruz)
  ...
  useEffect(() => {
      ...
      try {
        ...
        const results = await Promise.all(
          uniqueIds.map(async (aid) => {
            ...
            const sourceName =
              aid === yazarId ? (author?.isim || "—") : aliasIdToName[aid] || `#${aid}`;
            const extended: MergedWork[] = data.map((w) => ({
              ...w,
              _sourceAuthorId: aid,
              _sourceAuthorName: sourceName,
            }));
            return extended;
          })
        );
        ...
        // updated_at varsa ona göre; yoksa created_at; o da yoksa id DESC
        merged.sort((a, b) => {
    ...
    // includeAliases, aliasIds, aliasIdToName, author?.isim değişince tekrar
                ...
                >
                  <div className="flex items-start gap-3">
                    ...
                    <div className="min-w-0">
                      ...
                      {/* Mahlas etiketi (asıl yazar değilse göster) */}
