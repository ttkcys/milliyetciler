    ...
    <div className="min-h-screen bg-black text-white opacity-85">
      ...
      <div className="mx-auto max-w-7xl px-6 py-6 space-y-6">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <StatBox icon={<LibraryBig className="w-5 h-5" />} label="Dergiler" value={counters.dergi} />
          <StatBox icon={<Layers3 className="w-5 h-5" />} label="Sayılar" value={counters.sayi} />
          <StatBox icon={<FileText className="w-5 h-5" />} label="Yazılar" value={counters.yazi} />
          <StatBox icon={<Feather className="w-5 h-5" />} label="Yazarlar" value={counters.yazar} />
        </div>
            ...
            >
              <div className="flex items-center gap-2 text-white/80">
                ...
                <span className="text-white/50 text-xs">
                  (Dergi {facetTotals.dergi} • Sayı {facetTotals.sayi} • Yazı {facetTotals.yazi} • Yazar {facetTotals.yazar})
                </span>
              </div>
                ...
                {(["dergi", "sayi", "yazi", "yazar"] as const).map((k) => (
                  <label key={k} className="flex items-center gap-2 text-sm">
                    <input
                      ...
                      onChange={(e) => setTypeFilter((f) => ({ ...f, [k]: e.target.checked }))}
                    />
                    <span>{k[0].toUpperCase() + k.slice(1)}</span>
                  </label>
                ))}
                <button
                  onClick={() => setTypeFilter({ dergi: true, sayi: true, yazi: true, yazar: true })}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          {loading && page === 1
            ? Array.from({ length: 6 }).map((_, i) => (
                <div key={i} className="h-28 rounded-xl border border-[#333] bg-[#0f0f0f] animate-pulse" />
              ))
            : results.length === 0
